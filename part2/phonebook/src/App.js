...
const PersonForm = ({
  ...
  const isAlreadyAdded = (name) => {
    let arr = persons.map((person) =>
      Object.values(person).includes(name) ? true : false
    );
    return arr.includes(true) === true ? true : false;
  };
  ...
  const addNewPerson = (event) => {
    ...
    if (isAlreadyAdded(personObject.name) === true) {
      return (
        <>
          {window.confirm(
            `${personObject.name} is already added to the phonebook. Do you want to update number?`
          ) === true
      ...
      phonebookService
        ...
        .catch((error) => {
          setErrorMessage(`${error.response.data.split("<pre>")}`);
          setTimeout(() => {
            setErrorMessage(null);
          }, 5000);
        });
